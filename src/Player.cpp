...
{
	if((m_grow_player && !m_shrink_player) && (m_shape.getSize().x * m_shape.getSize().y < 10000))
		m_shape.setSize(sf::Vector2f(m_shape.getSize().x * 1.001, m_shape.getSize().y * 1.001));
	else if((!m_grow_player && m_shrink_player) && (m_shape.getSize().x * m_shape.getSize().y > 2500))
...
const float Player::get_player_size_percentage() const
{
	return (m_shape.getSize().x * m_shape.getSize().y) / 5000.0f;
}
